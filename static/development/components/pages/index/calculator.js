...
function calculate(e) {
    ...
    const excise = ({engine, year, fuel}) => {
        ...
        const cff_year = current_year - year > 15 ? 15 : current_year === year ? 1 : current_year - year - 1
    ...
    const excise_trucks = ({engine, year}) => {
        ...
        const cff = year_count === 0 ? 0.01 : year_count < 5 ? 0.02 : year_count < 8 ? 0.8 : 1
        return engine * cff
    }
    ...
    if (status) {
        ...
        const excise_result = category_value === 'trucks' ? excise_trucks({engine, year}) : excise({engine, year, fuel})
